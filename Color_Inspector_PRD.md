...
*This document defines the product requirements for Color Inspector, a Figma plugin that scans the current page to detect every color in use—both resolved hex values and design token references—and enables designers to search, filter, and select the elements where those colors are applied.*

# 1. Executive Summary

Color Inspector is a Figma plugin designed to give designers and design-system maintainers complete visibility into every color used on the current page—or within a specific selection. It detects both resolved (hard-coded hex/RGBA) colors and design token references (variables and styles), surfaces them in a searchable, filterable panel, and lets users select every element where a given color is applied—all without leaving Figma. When a user has a frame, section, or group selected before scanning, the plugin automatically limits its scope to that selection and its children, enabling targeted audits of individual components or sections without noise from the rest of the page.
...
| US-09  | Any User                 | select a frame, section, or group before scanning and have the plugin only analyze that subtree | I can audit a specific component or section without noise from the rest of the file |
...
|------------|--------------------------------------------------------------------------------------------------------------------------------------------|--------------|---------------------------------------------------------------------------------------------------------------------------------------------------------|
| FR-01      | Traverse the current page and all node types (frames, groups, components, instances, text, vectors, etc.)                                  | P0           | Use figma.currentPage.children and recurse                                                                                                              |
| FR-01a     | If the user has a frame, section, or group selected at scan time, limit traversal to that selection and its descendant children only       | P0           | Check figma.currentPage.selection on scan start                                                                                                         |
| FR-01b     | Display a visible scope indicator in the header showing whether the scan covers “Entire Page” or the name of the selected scope node       | P0           | Updates automatically when scope changes                                                                                                                |
...
| FR-04      | Extract text colors (character-level)                                                                                                      | P0           | A single text node can have multiple colors via ranges                                                                                                  |
...
| FR-07      | Detect Figma Styles bound to color properties                                                                                              | P0           | Use node.fillStyleId, strokeStyleId, etc.                                                                                                               |
...
| FR-12      | Sort by: usage count (default), hex value, token name, alphabetical                                                                                                                                                                                                                                           | P0           | Dropdown or segmented control                                                                           |
...
| FR-19      | Filter by property type: Fill, Stroke, Text, Effect (multi-select)                       | P1           | Checkbox pills                    |
| FR-21      | Clear all filters button                                                                 | P0           | Reset to default view             |
...
| FR-23      | Expand a color row to see a sub-list of individual elements (node name, page, property type)                                                                                                                                             | P0           | Lets user select individual elements                          |
...
The plugin UI is a single panel (recommended size: 360px wide × 560px tall) with the following vertical layout:

## 7.1 Panel Structure

1.  **Header Bar:** Plugin title (“Color Inspector”), settings gear icon, and a subtle “live” indicator. Below the title, a scope indicator chip: “Scope: Entire Page” or “Scope: [FrameName]” with an “×” to clear scope. No manual scan button needed—results update automatically.

2.  **Summary Strip:** Total colors count, token-bound count, hard-coded count, total elements scanned. Displayed as compact stat chips.

3.  **Search & Filter Bar:** Text input for search. Below it, a row of filter chips (Token-bound | Hard-coded | Fill | Stroke | Text | Effect).

4.  **Sort Controls:** Dropdown: “Sort by Usage (High → Low)” | “Sort by Hex” | “Sort by Token Name.”

5.  **Color List (scrollable):** Each row: 24px rendered swatch (always present; flat color for solids, actual gradient for gradients) → Text identifier (token name if bound, hex value if hard-coded; gradient type label for gradients) → Token/hard-coded badge → Usage count badge → “Select All” button. The swatch is the primary visual anchor for every row.

6.  **Expanded Element Sub-list:** When a color row is expanded, show child rows: Node name, Layer path, Property type (fill/stroke/text). Clicking zooms to the element.

7.  **Footer Bar:** Export button (JSON / CSV dropdown), “Help” link, version number.

## 7.2 Interaction States

| **State**                | **Behavior**                                                                                                                                                                                                                            |
|--------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Initial Load             | Plugin auto-scans on open. Shows a brief loading indicator while the first scan completes. If a frame/section/group is selected, the scope is set automatically.                                                                        |
...
| Virtual list | Custom or tanstack-virtual      | Required for smooth scrolling with 500+ color rows                    |
...
| Text node with mixed colors (styled ranges)                        | Extract each unique color used across character ranges. Associate all ranges with the same text node ID.                                                                                                                                                         |
| Hidden or locked layers                                            | Include by default. Provide a filter toggle to exclude hidden layers (P2).                                                                                                                                                                                       |
...
| User selects multiple nodes before scanning                        | Do not scope—fall back to full-page scan. Multiple selection is ambiguous and could confuse results. Show a brief tooltip: “Tip: select a single frame, section, or group to scope your scan.”                                                                   |
| User selects a leaf node (rectangle, text, vector) before scanning | Do not scope—fall back to full-page scan. Leaf nodes have no meaningful children to traverse. Show same tooltip as above.                                                                                                                                        |
...
- Selection scoping: select a frame, section, or group before scanning to limit audit to that subtree, with clear scope indicator and one-click reset
...
- Zoom-to-selection using Figma’s native selection highlight and scrollAndZoomIntoView for visual confirmation
...
| OQ-7   | When multiple frames/groups are selected, should we scan the union of all selected subtrees, or require a single selection?           | PM + Engineering | Leaning single-selection for v1.0 simplicity                                                                                         |
...
*__End of Document__ — Color Inspector PRD v1.0. For questions, reach out to the Product Management team.*
