...
_This document defines the product requirements for Color Me Good, a Figma plugin that scans the user’s selection to detect every color in use—both resolved hex values and design token references—and enables designers to search, filter, and select the elements where those colors are applied. Selection is required: the plugin only runs its scan when one or more elements are selected._

# 1. Executive Summary

Color Me Good is a Figma plugin designed to give designers and design-system maintainers complete visibility into every color used within a selection. The user must select one or more nodes (frames, groups, layers, etc.) in the canvas before the plugin runs; there is no full-page or “entire page” scan. It detects both resolved (hard-coded hex/RGBA) colors and design token references (variables and styles), surfaces them in a searchable, filterable panel, and lets users select every element where a given color is applied—all without leaving Figma. Selection-scoped scanning enables targeted audits of a single element, a frame, or multiple elements at once.
...
| ---------- | ------------------------------------------------------------------------------------------------------------------------------------------ | ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| FR-01      | Run the scan only when the user has one or more nodes selected; traverse the selected node(s) and all descendants; exclude vector types by default (optional "Include vectors" in Settings); skip hidden nodes | P0           | No scan when selection is empty; use figma.currentPage.selection as roots; node.visible === false excluded                                               |
| FR-01a     | Support any node type and multi-select as scope (frame, group, layer, etc.); each selected node is a traversal root                             | P0           | Check figma.currentPage.selection on scan start                                                                                                         |
| FR-01b     | Display a visible scope indicator in the header: "Select elements" when nothing is selected; otherwise the selected node name or "N elements" for multi-selection | P0           | Updates automatically when selection changes                                                                    |
| FR-01c     | Provide a "Clear selection" action that deselects the current scope and returns to the no-selection state (no scan)                                                       | P0           | One-click reset; deselects; plugin shows no-selection screen                                                                                                               |
...
The plugin UI is a single resizable panel (default 420×720; min 420×720, max 540×840). Resize from the right edge, bottom edge, or bottom-right corner. Vertical layout:

## 7.1 Panel Structure

1.  **Header Bar:** Plugin title (“Color Me Good”). Below the title, a scope indicator chip: "Scope: Select elements" when nothing is selected; "Scope: [NodeName]" or "N elements" for multi-selection when scoped, with an "×" to clear selection. No manual scan button—results update when selection changes.
...
| **State**                | **Behavior**                                                                                                                                                                                                                        |
| ------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| No selection (initial or after clear) | Dedicated screen with a line-art icon (e.g. pointer/select) and short guidance: "Select elements to scan" and "Select one or more frames, groups, or layers in the canvas. Colors from your selection will appear here." No scan runs. |
| Initial Load (with selection) | If one or more nodes are selected on open, plugin runs the scan for that scope and shows results. Otherwise, the no-selection screen is shown.                                                                                       |
| Scanning                 | Progress bar with node count (“Scanning… 4,218 / 12,000 nodes”). Cancel button available.                                                                                                                                           |
| Scoped Scan Active       | Scope indicator chip in the header shows "Scope: [NodeName]" or "N elements" with an "×" button. All results, counts, and exports reflect only the scoped subtree.                                                                 |
| Results Loaded           | Summary strip populates; color list renders. Search bar becomes active.                                                                                                                                                             |
| Selection but no colors  | Message: "No colors found in selection." with suggestion to try different or expanded selection.                                                                                                                                   |
...
1.  **Scope resolution:** The scan runs only when figma.currentPage.selection has one or more nodes. Use the selected node(s) as the traversal root(s); there is no full-page scan. Store the scope context (node ID(s), name(s), or "N elements") and send it to the UI thread for the scope indicator. When selection is empty, send a no-selection state so the UI shows the "Select elements to scan" screen.
...
| ------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| No selection                                                     | Show no-selection screen with icon and text: "Select elements to scan" and guidance to select one or more frames, groups, or layers. No scan runs.                                                                                                                |
| Selection made but no colors in scope                             | Show message: "No colors found in selection." with suggestion to try different or expanded selection.                                                                                                                                                             |
...
| Scoped node is deleted after scan                                  | The plugin detects the deletion via documentchange, clears the selection, and shows the no-selection state. Toast: "Scoped element was deleted. Select something to scan."                                                                                       |
| Scoped scan followed by "Clear selection"                          | Selection is cleared; plugin shows the no-selection screen. No scan runs.                                                                                                                                                                                        |
